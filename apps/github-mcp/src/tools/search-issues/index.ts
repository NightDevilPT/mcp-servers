import { z } from "zod";
import { IGithubSearchIssuesResponse } from "../../types/github-types";

export const SearchIssuesTool = {
  name: "search_issues",
  description: "Search issues and pull requests across GitHub repositories with filters for labels, state, authors, and more",
  schema: {
    inputSchema: {
      q: z.string().describe("Search query. Example: 'q=windows+label:bug+language:python'"),
  ...
  execute: async ({ q, sort, order, per_page, page }: {
    ...
    try {
      ...
      const data: IGithubSearchIssuesResponse = await response.json();
