import { z } from "zod";
import { IGithubSearchCodeResponse } from "../../types/github-types";

export const SearchCodeTool = {
  name: "search_code",
  description: "Search for code across all GitHub repositories with advanced filtering by repository, language, and file path",
  schema: {
    inputSchema: {
      q: z.string().describe("Search query. Example: 'q=addClass+repo:jquery/jquery'"),
  ...
  execute: async ({ q, sort, order, per_page, page }: {
    ...
    try {
      ...
      const data: IGithubSearchCodeResponse = await response.json();
